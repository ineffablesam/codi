...
- Signal router: Emits signals instead of direct delegation
- Attractor evaluator: Converges to stable states
- Artifact collector: Collects artifacts from agents
"""
...
from app.agents.mixins import ArtifactProducerMixin, SignalSubscriberMixin
from app.core.config import settings
from app.utils.logging import get_logger

# Antigravity imports
from app.core.artifacts import Artifact, ArtifactStore, get_artifact_store
from app.core.signals import Signal, SignalEvent, get_signal_engine, get_agents_for_signal
from app.core.attractors import AttractorEvaluator, get_all_attractors
...
class ConductorAgent(BaseAgent, ArtifactProducerMixin, SignalSubscriberMixin):
    """Master Orchestrator Agent with Antigravity capabilities.
    
    Combines traditional delegation with:
    - Signal routing: Emits signals instead of direct delegation
    - Attractor evaluation: Converges to stable states
    - Artifact collection: Collects artifacts from agents
    
    The Conductor can operate in two modes:
    1. Legacy mode: Direct delegation via delegate_task()
    2. Antigravity mode: Signal-based activation via run_until_attractors_satisfied()
    """
    ...
    def __init__(self, context: AgentContext) -> None:
        ...
        self._attractor_evaluator: Optional[AttractorEvaluator] = None
        self._collected_artifacts: List[Artifact] = []
    ...
    @property
    def attractor_evaluator(self) -> AttractorEvaluator:
        """Get or create attractor evaluator."""
        if self._attractor_evaluator is None:
            self._attractor_evaluator = AttractorEvaluator(self.artifact_store)
        return self._attractor_evaluator
    ...
    # ========================================================================
    # ANTIGRAVITY: Signal-Based Orchestration
    # ========================================================================
    
    async def handle_signal(self, event: SignalEvent) -> None:
        """Handle incoming signals (Antigravity mode).
        
        Args:
            event: Signal event to handle
        """
        logger.info(f"Conductor received signal: {event.signal.value}")
        
        if event.signal == Signal.INTENT_PARSED:
            # User intent was parsed, determine next signals
            await self._handle_intent_parsed(event)
        
        elif event.signal == Signal.PLAN_APPROVED:
            # Plan approved, start execution
            await self._handle_plan_approved(event)
        
        elif event.signal == Signal.PLAN_REJECTED:
            # Plan rejected, notify user
            await self._handle_plan_rejected(event)
        
        elif event.signal == Signal.ERROR_OCCURRED:
            # Error occurred, route to fixers
            await self._handle_error_occurred(event)
    
    async def _handle_intent_parsed(self, event: SignalEvent) -> None:
        """Handle parsed user intent."""
        intent_type = event.context.get("intent_type", "unknown")
        
        # Derive signals based on intent
        if intent_type in ["explicit", "open_ended"]:
            # Complex request - needs implementation
    ...
    async def _handle_plan_approved(self, event: SignalEvent) -> None:
    ...
    async def _handle_plan_rejected(self, event: SignalEvent) -> None:
        """Handle rejected plan."""
        logger.info("Plan was rejected, awaiting new instructions")
        # No automatic action - wait for new user input
    
    async def _handle_error_occurred(self, event: SignalEvent) -> None:
        """Handle error by routing to fixers."""
        error_type = event.context.get("error_type", "unknown")
        
        if error_type in ["build_error", "compile_error"]:
            await self.emit_signal(Signal.BUILD_FAILED, context=event.context)
        elif error_type == "test_failure":
            await self.emit_signal(Signal.TESTS_FAILING, context=event.context)
    
    async def run_until_attractors_satisfied(
        self,
        ...
        """
        Run the Antigravity convergence loop.
        
        This is the main entry point for signal-based orchestration.
        The loop:
        1. Evaluates attractors
        2. Derives signals from unsatisfied attractors
        3. Collects artifacts from signal handlers
        4. Repeats until all attractors satisfied (or timeout)
        
        Args:
            attractors: Attractors to satisfy (all if not specified)
            timeout: Maximum time in seconds
            poll_interval: Time between evaluations
            max_iterations: Maximum evaluation cycles
            
        Returns:
            Dict with convergence result
        """
        await self.emit_status(
            "started",
            "Starting Antigravity convergence loop..."
        )
        
        result = await self.attractor_evaluator.run_until_satisfied(
        ...
        if result.all_satisfied:
            await self.emit_status(
                "completed",
                "All attractors satisfied - system converged"
            )
        else:
            unsatisfied = self.attractor_evaluator.get_unsatisfied()
            await self.emit_status(
                "completed",
                f"Convergence ended with unsatisfied attractors: {unsatisfied}"
            )
        
        return {
            "converged": result.all_satisfied,
            "results": {
                name: r.status.value
                for name, r in result.results.items()
            },
            "signals_emitted": [s.value for s in result.signals_to_emit],
            "artifacts_collected": len(self._collected_artifacts),
        }
    
    async def emit_signal_instead_of_delegate(
        self,
        task_type: str,
        context: Dict[str, Any] = None,
    ) -> None:
        """
        Emit signals instead of direct delegation (Antigravity pattern).
        
        This replaces:
            delegate_task(agent="build_deploy", task="build project")
        
        With:
            emit_signal_instead_of_delegate("build")
        
        Args:
            task_type: Type of task (maps to signals)
            context: Task context
        """
        task_lower = task_type.lower()
        
        # Map tasks to signals
        if any(kw in task_lower for kw in ["build", "compile"]):
            await self.emit_signal(Signal.NEEDS_BUILD, context=context)
        
        elif any(kw in task_lower for kw in ["preview", "run", "start"]):
            await self.emit_signal(Signal.NEEDS_PREVIEW, context=context)
        
        elif any(kw in task_lower for kw in ["implement", "code", "create"]):
            await self.emit_signal(Signal.NEEDS_IMPLEMENTATION, context=context)
        
        elif any(kw in task_lower for kw in ["ui", "design", "style"]):
            await self.emit_signal(Signal.NEEDS_UI_DESIGN, context=context)
        
        elif any(kw in task_lower for kw in ["commit", "push", "git"]):
            await self.emit_signal(Signal.NEEDS_COMMIT, context=context)
        
        elif any(kw in task_lower for kw in ["review", "check"]):
            await self.emit_signal(Signal.CODE_REVIEW_NEEDED, context=context)
        
        elif any(kw in task_lower for kw in ["analyze", "explore"]):
            await self.emit_signal(Signal.NEEDS_ANALYSIS, context=context)
        
        else:
            # Default to implementation
            await self.emit_signal(Signal.NEEDS_IMPLEMENTATION, context=context)
    
    def collect_artifact(self, artifact: Artifact) -> None:
        """Collect an artifact produced by an agent."""
        self._collected_artifacts.append(artifact)
    
    def get_collected_artifacts(self) -> List[Artifact]:
        """Get all collected artifacts."""
        return self._collected_artifacts.copy()
    
    def clear_collected_artifacts(self) -> None:
