...
class StarterTemplateService:
    ...
    - flutter-serverpod: Flutter + Serverpod full-stack (Dart backend + Flutter frontend)
    ...
    FRAMEWORK_DIRS = {
        ...
        "flutter-serverpod": "flutter-serverpod",
        "react": "react-starter",
    ...
    FRAMEWORK_EXCLUDES = {
        ...
        "flutter-serverpod": {
            "dirs": {".dart_tool", ".idea", "build", ".git", "android", "ios", ".pub-cache", ".pub", "node_modules"},
            "files": {".metadata", "pubspec.lock", ".iml", ".DS_Store"},
        },
        "react": {
    ...
    def _apply_replacements(self, content: str, file_path: Path, project_name: str, project_title: str, package_name: str) -> str:
        ...
        elif self.framework == "flutter-serverpod":
            # Serverpod full-stack project - replace package names and project references
            # Handle dart files - update package imports
            if file_path.suffix == ".dart":
                content = content.replace(f"package:{{{{PROJECT_NAME}}}}_server/", f"package:{package_name}_server/")
                content = content.replace(f"package:{{{{PROJECT_NAME}}}}_client/", f"package:{package_name}_client/")
                content = content.replace(f"package:{{{{PROJECT_NAME}}}}_flutter/", f"package:{package_name}_flutter/")
            
            # Handle pubspec.yaml files - update names and paths
            if file_path.name == "pubspec.yaml":
                content = content.replace("name: {{PROJECT_NAME}}_server", f"name: {package_name}_server")
                content = content.replace("name: {{PROJECT_NAME}}_client", f"name: {package_name}_client")
                content = content.replace("name: {{PROJECT_NAME}}_flutter", f"name: {package_name}_flutter")
                content = content.replace("../{{PROJECT_NAME}}_client", f"../{package_name}_client")
