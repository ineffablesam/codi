...
class Mem0Service:
    ...
    def __new__(cls) -> "Mem0Service":
        """Singleton pattern for memory service with thread safety."""
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            if not api_key:
                logger.error("GEMINI_API_KEY not set - memory service disabled")
                self._memory = None
    ...
    async def add_memory(
        ...
        try:
            ...
            # Add memory to Mem0 (run in thread to avoid blocking)
    ...
    async def search_memories(
        ...
        try:
            # Search with Mem0 (run in thread to avoid blocking)
            ...
            for mem in (results or []):
                ...
                if project_id and str(mem_metadata.get("project_id")) != str(project_id):
                    continue
    ...
    async def delete_session_memories(
        ...
        try:
            # Get all user memories and filter by session
            all_memories = await self.get_all_user_memories(user_id=user_id, limit=1000)
            ...
            # Delete each memory (run in thread to avoid blocking)
            for mem in session_memories:
                mem_id = mem.get("id")
                if mem_id:
                    await asyncio.to_thread(self._memory.delete, mem_id)
            
            logger.info(f"Deleted {len(session_memories)} memories for session {session_id}")
            return True
    ...
    async def get_all_user_memories(
        ...
        try:
            # Get all memories (run in thread to avoid blocking)
            result = await asyncio.to_thread(
    ...
    async def process_conversation_for_memories(
        ...
        """Process conversation messages to extract and store important memories.
        
        Mem0 automatically identifies and extracts important information like
        user preferences, names, and key facts from conversations.
        ...
        """
        if not self.is_available:
            logger.warning("Memory service not available, skipping conversation processing")
            return []
        ...
        try:
            ...
            # Mem0's add() with messages format extracts facts automatically
            result = await asyncio.to_thread(
                self._memory.add,
                messages,
                user_id=user_id,
            ...
            if isinstance(result, dict):
                ...
                for r in result.get("results", []):
                    ...
                    if mem_id:
                        ...
                        logger.debug(f"Extracted memory: {r.get('memory', '')[:50]}...")
            ...
            if memory_ids:
                logger.info(f"Extracted {len(memory_ids)} memories from conversation for user {user_id}")
            
            return memory_ids
            
        except Exception as e:
            logger.error(f"Failed to process conversation for memories: {e}")
            return []
