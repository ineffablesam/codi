...
- Keep responses brief (2-3 sentences)"""
        ...
        if self.session_id:
            try:
                ...
                async with get_db_context() as session:
                    # Load all previous messages from this session
                ...
                if chat_messages:
                    logger.info(f"Loaded {len(chat_messages)} previous messages for conversational response")
                    
            except Exception as e:
                logger.warning(f"Failed to load chat history for conversational: {e}")
        ...
        try:
            ...
            # Save conversational response to database if session_id is provided
            if self.session_id:
                try:
                    ...
                    async with get_db_context() as session:
                        ...
                        logger.info(f"Saved conversational response to chat session {self.session_id}")
                        
                except Exception as e:
                    logger.warning(f"Failed to save conversational response to database: {e}")
