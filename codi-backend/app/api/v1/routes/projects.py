...
async def create_project(
    ...
    # For Serverpod backend, use the flutter-serverpod template
    template_framework = framework
    if backend_type == 'serverpod' and framework == 'flutter':
        template_framework = 'flutter-serverpod'

    # Create local Git repository
    git_service = get_git_service()
    project_slug = LocalGitService.slugify(project_data.name)
    
    # Generate Serverpod backend config if applicable
    backend_config_json = None
    if backend_type == 'serverpod':
        backend_config_json = json.dumps({
            "serverpod_url": f"http://{project_slug}-serverpod:8080/",
            "serverpod_public_url": f"http://localhost:8080/",
            "postgres_container": f"{project_slug}_postgres",
            "redis_container": f"{project_slug}_redis", 
            "server_container": f"{project_slug}_serverpod",
            "flutter_container": f"{project_slug}_flutter",
            "auto_managed": True,
        })
    ...
    try:
        ...
        template_service = StarterTemplateService(framework=template_framework)
    ...
    project = Project(
        ...
        backend_config=backend_config_json,  # Store Serverpod config for agent context
        # Status
    ...
    # For Serverpod projects, create default environment variables
    if backend_type == "serverpod":
        ...
        for key, config in defaults.items():
            ...
            env_var.set_value(config["value"], is_secret=config.get("is_secret", False))
            session.add(env_var)
        ...
        if env_vars:
            EnvironmentService.sync_to_file(
                project=project,
                variables=env_vars,
                context=None,  # Sync all variables
                include_secrets=True,
            )
            logger.info(f"Created and synced {len(env_vars)} default environment variables for Serverpod project")
