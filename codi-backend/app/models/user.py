...
class User(Base):
    ...
    # Opik AI tracing preferences (opt-in)
    opik_enabled: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False, index=True)
    opik_api_key: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # Encrypted, optional (user can use their own)
    opik_workspace: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "opik_enabled": self.opik_enabled,
            "opik_workspace": self.opik_workspace,
            # Note: opik_api_key is NOT included for security
